...
| `src/core/{cursors,webhooks,digests,protocols}.ts` | Durable per-agent cursors; outbound HTTP with HMAC + auto-disable; agent-driven compaction; versioned playbooks. Webhook *delivery* and digest *scheduling* are leader-only (armed via `activate()`/`start()` from the CoordinatorLease); add/remove/list work on any replica. Agent-scoped **wakes** ride the same engine (`kind`/`owner`/`debounceSec`, source-side debounce, wake-envelope body, quota enforced in the tool layer). |
